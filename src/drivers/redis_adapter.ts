...
export class RedisAdapter implements Adapter {
   ...
   async pushLaterOn(
      ...
      if (jobData.dedup) {
         ...
         const [payloadData, payloadIsUndefined] = encodeRedisJobPayloadOverlay(
            jobData.payload,
         );
         const result = (await this.#connection.eval(
            PUSH_DEDUP_JOB_SCRIPT,
   ...
   async pushManyOn(queue: string, jobs: JobData[]): Promise<void> {
      ...
      if (jobs.some((j) => j.dedup)) {
         throw new Error(
            'dedup is not supported in batch dispatch; use single dispatch',
         );
      }
   ...
   async upsertSchedule(config: ScheduleConfig): Promise<string> {
      ...
      const [existingRunCount, existingCreatedAt, existingNextRunAt] =
         await this.#connection.hmget(
            scheduleKey,
            'run_count',
            'created_at',
            'next_run_at',
      ...
      if (config.everyMs !== undefined)
         scheduleData.every_ms = config.everyMs.toString();
      if (config.from !== undefined)
         scheduleData.from_date = config.from.getTime().toString();
      if (config.to !== undefined)
         scheduleData.to_date = config.to.getTime().toString();
      if (config.limit !== undefined)
         scheduleData.run_limit = config.limit.toString();
      ...
      if (existingNextRunAt) {
         multi.zadd(
            schedulesDueKey,
            Number.parseInt(existingNextRunAt, 10),
            id,
         );
      }
   ...
   async listSchedules(options?: ScheduleListOptions): Promise<ScheduleData[]> {
      ...
      for (const [, data] of results) {
         ...
         const schedule = this.#hashToScheduleData(
   ...
   async updateSchedule(
      ...
      if (updates.nextRunAt !== undefined) {
         data.next_run_at = updates.nextRunAt
            ? updates.nextRunAt.getTime().toString()
            : '';
      }
      if (updates.lastRunAt !== undefined) {
         data.last_run_at = updates.lastRunAt
            ? updates.lastRunAt.getTime().toString()
            : '';
      }
      if (updates.runCount !== undefined)
         data.run_count = updates.runCount.toString();
      ...
      if (updates.status === 'active' && updates.nextRunAt === undefined) {
         const existing = await this.#connection.hget(
            scheduleKey,
            'next_run_at',
         );
         if (existing) {
   ...
   async claimDueSchedule(): Promise<ScheduleData | null> {
      ...
      if (data.cron_expression) {
         ...
         const runLimit = data.run_limit
            ? Number.parseInt(data.run_limit, 10)
            : null;
   ...
   #hashToScheduleData(data: Record<string, string>): ScheduleData {
      return {
         ...
         from: data.from_date
            ? new Date(Number.parseInt(data.from_date, 10))
            : null,
         ...
         nextRunAt: data.next_run_at
            ? new Date(Number.parseInt(data.next_run_at, 10))
            : null,
         lastRunAt: data.last_run_at
            ? new Date(Number.parseInt(data.last_run_at, 10))
            : null,
         status: (data.status as 'active' | 'paused') || 'active',
         createdAt: data.created_at
            ? new Date(Number.parseInt(data.created_at, 10))
            : new Date(),
      };
   }
}
